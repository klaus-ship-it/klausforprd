...
import { frontendBlacklistApi } from '@/api/whitelist'
...
const load = async () => {
  ...
  const res = await frontendBlacklistApi.list()
  if (res.code === 0) {
...
const handleBatchSave = async () => {
  ...
  try {
    const promises = Object.entries(pendingChanges.value).map(([id, enabled]) => 
      frontendBlacklistApi.update(id, { enabled })
    )
...
const save = async () => {
  ...
  if (editing.value) {
    const res = await frontendBlacklistApi.update(editing.value.id, formModel)
    if (res.code === 0) message.success('已更新')
    else message.error(res.msg)
  } else {
    const res = await frontendBlacklistApi.create(formModel)
...
const remove = async (id: string) => {
  ...
  const res = await frontendBlacklistApi.remove(id)
  if (res.code === 0) {
    message.success('已刪除')
    load()
  } else {
...
<template>
  <div class="p-6">
    ...
    <NCard title="前台黑名單管理" class="mb-4">
      <template #header-extra>
        <div class="flex gap-2">
          ...
          <NButton type="primary" @click="openCreate">新增黑名單</NButton>
        </div>
      </template>
    ...
    <NModal v-model:show="showModal" title="黑名單設定" preset="card" style="width:640px">
